...
describe('createRecipe', () => {
  it('creates a recipe with a title only', async () => {
    const recipe = await createRecipe(TEST_USER_ID, { title: 'Pasta', tags: [] })
  ...
  it('reuses existing tags across recipes rather than creating duplicates', async () => {
    ...
    const tags = await prisma.tag.findMany({ where: { name: 'main', userId: TEST_USER_ID } })
...
describe('updateRecipe', () => {
  it('updates title and optional fields', async () => {
    const recipe = await createRecipe(TEST_USER_ID, { title: 'Old Title', tags: [] })
  ...
  it('replaces all tags on update', async () => {
    ...
    await updateRecipe(recipe.id, TEST_USER_ID, { title: 'Recipe', tags: ['main'] })
    const updated = await findRecipe(recipe.id, TEST_USER_ID)
    expect(updated?.tags.map(({ tag }) => tag.name)).toEqual(['main'])
  })

  it('removes all tags when updated with empty tag list', async () => {
    const recipe = await createRecipe(TEST_USER_ID, { title: 'Recipe', tags: ['main'] })
  ...
  it('clears an optional field when passed an empty string', async () => {
    ...
    await updateRecipe(recipe.id, TEST_USER_ID, { title: 'Recipe', ingredients: '', tags: [] })
    const updated = await findRecipe(recipe.id, TEST_USER_ID)
...
describe('removeRecipe', () => {
  it('deletes the recipe', async () => {
    const recipe = await createRecipe(TEST_USER_ID, { title: 'To Delete', tags: [] })
  ...
  it('cascades deletion to notes', async () => {
    ...
    await removeRecipe(recipe.id, TEST_USER_ID)
    const notes = await prisma.note.findMany({ where: { recipeId: recipe.id } })
...
describe('notes', () => {
  ...
  it('throws when adding a note to a recipe the user does not own', async () => {
    const recipe = await createRecipe(TEST_USER_ID, { title: 'Recipe', tags: [] })
    await expect(addNote(recipe.id, 'other-user-id', 'Sneaky note')).rejects.toThrow(
      'Recipe not found',
    )
  })
})
